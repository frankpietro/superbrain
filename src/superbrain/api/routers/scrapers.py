...
async def status(lake: Annotated[Lake, Depends(get_lake)]) -> ScrapersStatus:
    """Aggregate latest-run + last-24h counters per bookmaker."""
    blocks = await anyio.to_thread.run_sync(_status_sync, lake)
    return ScrapersStatus(bookmakers=blocks)
...
def _status_sync(lake: Lake) -> list[ScrapersStatusBookmaker]:
    ...
    for slug in _KNOWN_BOOKMAKERS:
        ...
        errors_24h = 0
        if not sub.is_empty():
            last = sub.sort("started_at", descending=True).head(1).row(0, named=True)
            ...
            if recent.height:
                ...
                errors_24h = int(recent.filter(pl.col("status") != "ok").height)
        blocks.append(
            ScrapersStatusBookmaker(
                bookmaker=slug,
                last_run=last_row,
