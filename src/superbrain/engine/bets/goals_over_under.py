"""Goals — total over/under bet.

Mirrors ``fbref24/refactored_src/bets/goals_over_under.py``. Probability
of OVER(threshold) is the fraction of neighbor matches where
``home_goals + away_goals >= threshold``. UNDER is the complement
(strict ``<``).
"""
...
@register(Market.GOALS_OVER_UNDER)
class GoalsOverUnderBet:
    ...
    def iter_outcomes(self, odds: Iterable[OddsSnapshot]) -> Iterable[Outcome]:
        ...
        for o in odds:
            if o.market != Market.GOALS_OVER_UNDER:
                continue
