...
def test_list_odds_default_order(
...
def test_odds_bookmaker_filter(client: TestClient, lake: Lake, auth_header: dict[str, str]) -> None:
    _seed_odds(lake)
    resp = client.get("/odds", params={"bookmaker": "goldbet"}, headers=auth_header)
    assert resp.status_code == 200
    body = resp.json()
    assert body["count"] == 1
...
def test_odds_market_and_captured_from(
    client: TestClient, lake: Lake, auth_header: dict[str, str]
) -> None:
    _seed_odds(lake)
    resp = client.get(
...
def test_odds_limit_rejects_over_cap(
    client: TestClient, lake: Lake, auth_header: dict[str, str]
) -> None:
    _seed_odds(lake)
    resp = client.get("/odds", params={"limit": 10000}, headers=auth_header)
    assert resp.status_code == 422


def test_odds_match_id_filter_respects_date(
    client: TestClient, lake: Lake, auth_header: dict[str, str]
) -> None:
    _seed_odds(lake)
    resp = client.get("/odds", params={"match_id": "none-match"}, headers=auth_header)
    assert resp.status_code == 200
